...
- `cover` — имя файла обложки внутри папки галереи (например, `"1.jpg"`)

### Баннер первого визита

Настройки в `content/welcome-banner.json`:

- `enabled` — включить/выключить баннер
- `title`, `description` — текст предложения
- `ctaUrl`, `ctaLabel` — ссылка и кнопка (по умолчанию Telegram-канал)
- `dismissLabel` — текст кнопки «позже»
- `storageKey` — ключ в `localStorage`; смените его, если нужно показать баннер повторно всем пользователям
